-- Admin link = no partner commission (admin keeps it all)
-- Delton R15 on delton + all
-- Richard R20 on richard + all
...
begin
  ...
  if new.order_type in ('delton', 'all') and delton_id is not null then
...
begin
  ...
  if typ in ('delton', 'all') and delton_id is not null then
    insert into public.order_splits (order_id, user_id, amount)
...
-- Delton no longer linked to admin-only orders
...
-- Rebuild splits: remove Delton from admin-only groups
...
where g.order_type in ('delton', 'all');
...
where g.order_type in ('richard', 'all');
